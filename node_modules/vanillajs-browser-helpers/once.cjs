...
const eventOptionsSupported_1 = __importDefault(require("./eventOptionsSupported"));
...
const manuelOnce = (elm, handler, when) => {
    const offHandler = (e) => {
        ...
        (0, off_1.default)(elm, e.type, offHandler);
...
const bind = (elm, eventNames, handler, options) => {
    const noOptions = !(0, eventOptionsSupported_1.default)();
    const _a = options || {}, { when } = _a, eventOptions = __rest(_a, ["when"]);
    eventOptions.once = !when;
    const eventHandler = when || noOptions
        ? manuelOnce(elm, handler, when)
        : handler;
    (0, on_1.default)(elm, eventNames, eventHandler, eventOptions);
    return () => (0, off_1.default)(elm, eventNames, eventHandler, eventOptions);
};
function once(elm, eventNames, handler, options) {
    if (!(0, isEventTarget_1.default)(elm)) {
        options = handler;
        handler = eventNames;
        eventNames = elm;
        elm = document;
